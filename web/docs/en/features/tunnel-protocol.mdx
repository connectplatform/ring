...
| Peer games inbox | `games:incoming` | `publishToUserTunnel(peer, 'games:incoming', …)` | `IncomingGameBanner` via `GlobalTunnelListeners` (app-wide) |
...
<FutureFeatureWidget
  ...
  description="Shipped + smoked (2026-08-05) then **replicas reverted to 1** (2026-08-06): dual Next pods thrashed single-node k3s-or (4 vCPU / 8Gi). Keep `TUNNEL_POSTGRES_FANOUT=disabled` at 1 replica (restores offline queue; live-only is multi-pod only). Cookie affinity armed. Re-enable fan-out + temporary 2-replica smoke only when the node has headroom (`scripts/ci/smoke-tunnel-postgres-fanout.sh`). Redis/Connect hubs remain backlog."

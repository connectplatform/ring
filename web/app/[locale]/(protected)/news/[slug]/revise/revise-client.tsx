...
export function ReviseNewsClient({
  ...
  const initialWasLegacyHtml = useMemo(
    () => resolveNewsBodyMode(initialContent) === 'legacy_html',
    [initialContent],
  )
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 p-4">
      ...
      {initialWasLegacyHtml ? (
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>{t('editor.legacyHtmlBanner')}</AlertDescription>
        </Alert>
      ) : null}
      ...
      <NewsRichEditor
        ...
        selectionMemoryScope={`news_revise_${articleId}`}

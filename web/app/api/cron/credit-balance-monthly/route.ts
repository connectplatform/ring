/**
 * Cron Job: Credit Balance Monthly Renewal
 *
 * Runs daily to auto-charge credit_balance subscribers and handle failures.
 *
 * Vercel Cron:
 * { "path": "/api/cron/credit-balance-monthly", "schedule": "0 * * * *" }
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const handler = getPipelineDefinition('credit-balance-monthly')?.handler
    if (!handler) return NextResponse.json({ error: 'Pipeline not registered' }, { status: 500 })
    const { result, run } = await ProcessConductor.recordRun('credit-balance-monthly', 'cron', handler)
    if (run.status === 'error') {

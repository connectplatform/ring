...
export async function createVendorStore(prevState: any, formData: FormData) {
  ...
  try {
    ...
    // Non-blocking storefront research after id is known (do not block onboarding).
    if (existingStorefrontUrls.length > 0) {
      const firstUrl = existingStorefrontUrls[0]
      void import('@/lib/web/conductor')
        .then(({ WebConductor }) =>
          WebConductor.researchStorefront({
            storefrontUrl: firstUrl,
        ...
        .then((research) => {
          console.info(
            JSON.stringify({
              tag: 'web-conductor.storefront',
              entityId,
              storefrontUrl: firstUrl,
              citations: research.citations?.length ?? 0,
              images: research.imageCandidates?.length ?? 0,
            }),
          )
        })
        .catch((err) => {
          console.warn('vendor start: researchStorefront failed', err)
        })
    }

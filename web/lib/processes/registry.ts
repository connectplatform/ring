...
import { runProductAgentEnrich } from '@/lib/processes/product/product-agent-enrich'
...
export const PIPELINE_REGISTRY: PipelineDefinition[] = [
  ...
  {
    id: 'product-agent-enrich',
    category: 'ai',
    cronPath: '/api/cron/product-agent-enrich',
    handler: async () => {
      const startTime = Date.now()
      const report = await runProductAgentEnrich(5)

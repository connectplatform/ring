...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.97.17-blue?style=flat-square" alt="Version" />
  ...
  <img src="https://img.shields.io/badge/Open_Source-Community-green?style=flat-square" alt="Open Source" />
  <img src="https://img.shields.io/badge/License-Open_Source-green?style=flat-square" alt="License" />
</p>

---

React 19 + Next.js 16 open-source platform: AI-Matcher, SSE-push tunnel, direct messaging, rtc/turn, opportunities pool, mvm-market, vendor-store, user-credit, karpathy-wiki, file cabinet, solana/evm, own token, credit/token-swap, staking, nft-gate, nft-market. Clone free from GitHub and self-host on your infrastructure.

This repository is the **community Layer1** codebase (`ring` / `ring/web`). Managed hosting, settler ringization, and empire overlays live in a separate overlay repo — not required to run or customize Ring yourself.

**Version**: 1.97.17 | **Site**: [ring-platform.org](https://ring-platform.org) | **Changelog**: [/changelog](https://ring-platform.org/changelog) | **Origin**: Built by Ray Sorkin, a free person of Ukraine

## Self-host path

| Path | Who | How |
|------|-----|-----|
| **Clone & self-host** | Developers, communities, organizations | Clone [github.com/connectplatform/ring](https://github.com/connectplatform/ring), run `install.sh` / `scripts/setup-dev.sh`, set `ring-config`, deploy on your stack. |
| **Customize a vertical** | Product teams | Use presets (`entities.preset`, product fields/badges), locales, and `ring-config` — keep overlays thin. |

### Stack shape

```
Ring Platform (this repo — OSS community)
  |-- install.sh / setup-dev.sh
  |-- ring-config + locales
  |-- Next.js 16 app (ring/web)
```


## What's New in v1.97.17 (August 2026)

Full history: [ring-platform.org/changelog](https://ring-platform.org/changelog). Newest highlights:

- **Share & Earn username tags** — Share any page as `path#username`; hash claim stamps `ring_ref`; assisted sales pay `credit` units
- **Personal page / Page Builder UX** — URL copy + status dot + fullscreen builder; save overlay (no toggle flicker)
- **Docs FS isolation + i18n (1.97.16)** — Pass-through docs layout; locale-aware loading/not-found
- **Postgres Tunnel fan-out (1.97.15)** — Optional LISTEN/NOTIFY multi-pod publish (default off)
- **Personal page RIP3 (1.97.14)** — Blocklist, NFT/media pins, private-view analytics
- **Foundation** — File Cabinet, CRM lab, PaymentConductor, Tunnel, AI Matcher, multi-vendor store, DE/ES locales

> **Note:** The Ring CLI (`ring` command) is **not** included in the public OSS tree. Community developers use [`install.sh`](install.sh) and [`scripts/setup-dev.sh`](scripts/setup-dev.sh). Enterprise/K8s compose tooling ships with managed overlays when you use a hosted deployment path. See also [FEATURESET.md](FEATURESET.md) and [ROADMAP.md](ROADMAP.md).
...
### Revenue Streams
- **Membership & credits**: optional MEMBER tiers and RING-token rails (configure per clone)
...
3. Customize committed `web/ring-config.json` (or overwrite from template for a new brand)
...
```bash
# Clone the repository (Final-Split: Next app is under web/)
...
# ring-config.json is committed with localhost community defaults.
# Edit web/ring-config.json for branding, or for a greenfield clone:
#   cp web/ring-config.template.json web/ring-config.json

# Run development server (from ring/ root)
...
**Legiox AI**: Truth-lens skillsets and MCP tools for Ring development (optional IDE plugin path for operators who adopt it).
...
- **Example live clones**:
  - [app.greenfood.live](https://app.greenfood.live) — multi-vendor marketplace
  - [app.vikka.ua](https://app.vikka.ua) — media / news
  - [zemna.ai](https://zemna.ai) — scientific publication
  - [ring.ck.ua](https://ring.ck.ua) — regional opportunities
  - [ring-platform.org](https://ring-platform.org) — community portal

---

<p align="center">
  <em>Ring Platform — open-source white-label collaboration stack. Clone. Customize. Self-host.</em>
</p>

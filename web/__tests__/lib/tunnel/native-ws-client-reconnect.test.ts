...
describe('NativeWsClient auto-reconnect', () => {
  ...
  it('force-closes socket when heartbeat pong never arrives', async () => {
    const client = new NativeWsClient({
      url: 'ws://localhost/api/tunnel/ws',
      reconnectDelay: 1000,
      maxReconnectAttempts: 5,
    ...
    await flushMicrotasks()
    expect(first.sent.some((s) => s.includes('"op":"ping"'))).toBe(true)

    // No pong — watchdog should force-close.
    await jest.advanceTimersByTimeAsync(1_000)
    await flushMicrotasks()
    expect(closeSpy).toHaveBeenCalled()
    expect(reconnects).toEqual([1])
    expect(client.getState().status).toBe('reconnecting')
    expect(client.getState().lastError).toMatch(/heartbeat timeout/i)
  })

  it('kicks reconnect on visibilitychange when socket is not open', async () => {
    const visibilityHandlers: Array<EventListenerOrEventListenerObject> = []
    const docAdd = jest.spyOn(document, 'addEventListener').mockImplementation((type, handler) => {
      if (type === 'visibilitychange') visibilityHandlers.push(handler)
    })
    const docRemove = jest.spyOn(document, 'removeEventListener').mockImplementation(() => {})
    const winAdd = jest.spyOn(window, 'addEventListener').mockImplementation(() => {})
    const winRemove = jest.spyOn(window, 'removeEventListener').mockImplementation(() => {})
    ...
    try {
      ...
      const kicks: string[] = []
      client.on('reconnect-kick', ({ source }: { source: string }) => {
        kicks.push(source)
      })
      ...
      // Simulate half-open / dropped link without going through reconnect yet.
      ...
      for (const handler of visibilityHandlers) {
        if (typeof handler === 'function') {
          ...
          handler.handleEvent(new Event('visibilitychange'))
        }
      }
      await flushMicrotasks(16)

      expect(kicks).toContain('visibilitychange')

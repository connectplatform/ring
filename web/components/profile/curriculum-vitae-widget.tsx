...
import { BorderBeam, davinciGlassSurface } from '@/lib/ui/davinci'
...
export function CurriculumVitaeWidget({
  ...
  const [saveExitBeam, setSaveExitBeam] = useState(false)
  ...
  // Pulse BorderBeam on Save&Exit when a draft write settles (editor beam removed).
  useEffect(() => {
    if (!open) {
      setSaveExitBeam(false)
      return
    }
    if (debouncedDraft !== draft) return
    setStoredDraft(draft)
    const dirty =
      draft.trim() !== (cv.markdown || '').trim() && draft.trim().length > 0
    if (!dirty) return
    setSaveExitBeam(true)
    const t = window.setTimeout(() => setSaveExitBeam(false), 2800)
    return () => window.clearTimeout(t)
  }, [open, draft, debouncedDraft, setStoredDraft, cv.markdown])
  ...
  return (
    <div className={cn(davinciGlassSurface, 'space-y-4 p-4 sm:p-5', className)}>
      ...
      <FsModal
        ...
        hideTitleOnMobile
        hideCloseButton
        // Keep center-pane left/right insets (dynamic width); floor only so it never gets tiny.
        className="max-lg:!pt-0 lg:min-w-[600px]"
        headerClassName="max-sm:hidden sm:block !space-y-0 !px-2 !pb-0 !pt-1.5 sm:!px-2"
        ...
        footerClassName="!px-0 border-t border-border/60 max-lg:pb-[max(0.75rem,env(safe-area-inset-bottom))] py-2 sm:py-2"
        footer={
          <div className="flex w-full items-center gap-2 px-1.5 sm:px-2">
            ...
            <div className="ml-auto flex items-center gap-3">
              ...
              <BorderBeam
                className="wiki-save-exit-beam"
                borderOnly
                duration="2.4s"
                disabled={!saveExitBeam}
                innerClassName="!rounded-[99px] !p-0 bg-transparent"
              >
                <Button
                  type="button"
                  disabled={pending}
      ...
      >
        <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden max-sm:h-full">
          <WikiRichEditor
            ...
            closeOnToolbarMobileOnly
            autoFocus
            selectionMemoryScope={userId ? `cv_${userId}` : 'cv_anon'}

...
#   (Final-Split: launcher at repo root delegates to web/; or: cd ring/web && ./install.sh)
...
# Final-Split: committed ring-config.json is Layer1 SSOT; seed from template only if missing
if [[ ! -e "${SCRIPT_DIR}/ring-config.json" ]]; then
  if [[ -f "${SCRIPT_DIR}/ring-config.template.json" ]]; then
    cp "${SCRIPT_DIR}/ring-config.template.json" "${SCRIPT_DIR}/ring-config.json"
  fi
fi
...
step_4_ring_config() {
    ...
    if [[ -f "$config_file" ]]; then
        if ! prompt_destructive "ring-config.json exists (Layer1 ships committed community defaults). Overwrite for a new clone brand?"; then

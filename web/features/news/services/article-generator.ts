...
const ARTICLE_DRAFT_SCHEMA = {
  ...
  schema: {
    ...
    properties: {
      title: { type: 'string' },
      content: { type: 'string', description: 'Markdown article body (headings, paragraphs, links; no raw HTML)' },
...
function appendSourcesFooter(content: string, citations: string[] | undefined): string {
  ...
  const items = unique.map((url) => `- [${url}](${url})`).join('\n')
  return `${content}\n\n## Sources\n\n${items}\n`
}
...
function buildResearchPrompt(input: GenerateArticleInput): string {
  const lines = [
    ...
    'Return JSON matching the schema. Content must be valid Markdown (paragraphs, headings, links). Do not emit raw HTML tags.',

...
import { resolveNewsBodyMode } from '@/features/news/lib/render-news-markdown';

const NewsRichEditor = dynamic(
  () =>
    import('@/features/news/components/news-rich-editor').then((m) => m.NewsRichEditor),
  {
    ssr: false,
    loading: () => (
...
export function ArticleEditor({
  ...
  // Tip content autosave — only after user edits (avoid idle mount PUT)
  ...
  const triggerContentAutosave = useCallback(
    async (markdown: string) => {
      if (mode !== 'edit' || !article?.id || viewingHistorical || !markdown.trim()) return;
    ...
    [article?.id, mode, viewingHistorical],
  );

  useEffect(() => {
    if (mode !== 'edit' || !article?.id || viewingHistorical) return;
    if (!contentDirtyRef.current) return;
    if (autosaveTimerRef.current) clearTimeout(autosaveTimerRef.current);
    autosaveTimerRef.current = setTimeout(() => {
      void triggerContentAutosave(formData.content);
    }, 3000);
    return () => {
      if (autosaveTimerRef.current) clearTimeout(autosaveTimerRef.current);
    };
    ...
    formData.content,
    mode,
    triggerContentAutosave,
    viewingHistorical,
  ]);

  const viewingCommit = versionCommits[viewingCommitIndex];
  const viewingLegacyHtml =
    Boolean(formData.content) &&
    resolveNewsBodyMode(formData.content, viewingCommit?.contentFormat) ===
      'legacy_html';
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {/* Content — NewsRichEditor (Markdown SSOT + widgets) */}
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {autoSaveError ? (
                    <span className="text-sm text-destructive">{autoSaveError}</span>
                  ) : autoSaveLabel ? (
                    <span className="text-sm text-muted-foreground">
                      {tNews('editor.autoSavedAt', { time: autoSaveLabel })}
                    </span>
                  ) : null}
                </div>
                ...
                {viewingLegacyHtml && !previewMode ? (
                  <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription>
                      {tNews('editor.legacyHtmlBanner')}
                    </AlertDescription>
                  </Alert>
                ) : null}
                {previewMode || (viewingLegacyHtml && viewingHistorical) ? (
                  <div className="min-h-[320px] rounded-md border border-border p-3">
                    <NewsMarkdownView
                      content={formData.content}
                      contentFormat={viewingCommit?.contentFormat}
                  ...
                  <NewsRichEditor
                    ...
                    selectionMemoryScope={
                      article?.id
                        ? `news_${article.id}`
                        : session?.user?.id
                          ? `news_draft_${session.user.id}`
                          : 'news_anon'
                    }
                    className="min-h-[320px]"
                  />
                )}
                <p className="text-sm text-gray-500">{tNews('editor.markdownHint')}</p>
              </div>
            </CardContent>
          </Card>

...
/** Normalize article body for line diffs — HTML tips strip tags; Markdown tips pass through. */
export function contentToDiffText(content: string): string {
  const raw = content || ''
  const trimmed = raw.trim()
  if (/^<[a-zA-Z!/]/.test(trimmed)) {
    return htmlToDiffText(raw)
  }
  return raw.replace(/\r\n/g, '\n').trim()
}

/** @deprecated Prefer contentToDiffText — kept for HTML revision callers. */

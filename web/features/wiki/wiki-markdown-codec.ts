...
export function markdownToEditorHtml(
  markdown: string,
  options?: { preserveNewsShortcodes?: boolean },
): string {
  ...
  // News shortcodes [[mood:…]] / [[embed:…]] / [[video:…]] stay literal when preserved.
  const stubbed = mapWikiLinksOutsideCode(markdown || '', (_full, inner) => {
    if (
      options?.preserveNewsShortcodes &&
      /^(mood|embed|video):/i.test(inner.trim())
    ) {
      return _full
    }

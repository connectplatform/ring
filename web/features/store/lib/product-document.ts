 ...
 * Empty/missing listStores (legacy Salesbox/GreenFood imports) defaults to main catalog.
 */
export function isListedOnMainStore(doc: ProductDoc): boolean {
  const stores = resolveListStores(doc)
  if (stores.length === 0) return true
  return stores.includes(MAIN_STORE_ID)
}

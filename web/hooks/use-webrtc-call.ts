...
import { useCallSessionOptional } from '@/features/chat/providers/call-session-provider'
...
export type UseWebRtcCallOptions = {
   ...
   * CallSessionProvider binds active PC identity across soft-nav.
   */
...
export type UseWebRtcCallReturn = {
  ...
  toggleAudio: () => void
  toggleVideo: () => void
  shareScreen: () => Promise<{ ok: boolean; error?: string }>
...
export function useWebRtcCall(options: UseWebRtcCallOptions): UseWebRtcCallReturn {
  ...
  // Device picker: listMediaDevices(); screen share: shareScreen/stopScreenShare.
  ...
  const startCall = useCallback(
    async (m: CallMedia): Promise<{ ok: boolean; error?: string }> => {
      ...
      try {
        ...
        // retries: 0 — invite system line is setNxPx-deduped server-side; avoid client double-post.
        // Offline peer: call-invite route sends CALL_INVITE via notification dual-stack (FCM + web-push).
  ...
  const shareScreen = useCallback(async () => {
    try {
      ...
      if (!displayTrack) {
        return { ok: false, error: 'No display track' }
      }
      const currentVideo = localRef.current.getVideoTracks()[0] || null
      cameraTrackRef.current = currentVideo
      const sender = pc.getSenders().find((s) => s.track?.kind === 'video')
      if (sender) {
        await sender.replaceTrack(displayTrack)
      } else {
        pc.addTrack(displayTrack, localRef.current)
      }
      if (currentVideo) {
        localRef.current.removeTrack(currentVideo)
      }
      localRef.current.addTrack(displayTrack)
      ...
      return {
        ok: false,
        error: e instanceof Error ? e.message : 'Screen share failed',
      }
    }
  }, [stopScreenShare])
  ...
  // CallSessionProvider can keepAlive for same-conversation soft-nav; cross-thread still hangs up.
  const callSession = useCallSessionOptional()
  const activeCallConversationRef = useRef<string | null>(null)
  useEffect(() => {
    if (phase !== 'idle' && phase !== 'ended') {
      callSession?.bindActiveCall({
        conversationId,
        peerUserId,
        callId: callIdRef.current,
        media: mediaRef.current,
      ...
      if (
        ...
        const shouldHangup =
          callSession?.shouldHangupOnConversationChange(conversationId, phase) ??
          true
        if (shouldHangup) {
          void hangup()
        }
      ...
      callSession?.clearActiveCall()
    }
  }, [callSession, conversationId, hangup, peerUserId, phase])

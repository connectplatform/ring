 ...
 *
 * TODO(i18n): inline UA/EN copy (categories, hero slides, trust badges) should migrate to
 * locales pages.home.mvm* keys — preserved verbatim from GreenFood for cutover fidelity.
 */
const HomeContent: React.FC<HomeContentProps> = ({ session }) => {
  ...
  // Trending products (random selection)
  ...
  const categories = [
    { id: 'organic', name: 'Органічні продукти', nameEn: 'Organic Products', icon: '🌱', gradient: 'from-emerald-500 via-green-500 to-teal-500', count: 120 },
    { id: 'dairy', name: 'Молочні продукти', nameEn: 'Dairy Products', icon: '🥛', gradient: 'from-sky-400 via-blue-500 to-cyan-500', count: 85 },
    { id: 'meat', name: "М'ясо та птиця", nameEn: 'Meat & Poultry', icon: '🥩', gradient: 'from-rose-500 via-red-500 to-pink-500', count: 64 },
    { id: 'vegetables', name: 'Овочі та зелень', nameEn: 'Vegetables & Greens', icon: '🥬', gradient: 'from-lime-500 via-green-500 to-emerald-500', count: 156 },
    { id: 'fruits', name: 'Фрукти та ягоди', nameEn: 'Fruits & Berries', icon: '🍎', gradient: 'from-orange-500 via-amber-500 to-yellow-500', count: 98 },
    { id: 'honey', name: 'Мед та бджільництво', nameEn: 'Honey & Bee Products', icon: '🍯', gradient: 'from-amber-400 via-yellow-500 to-orange-400', count: 42 },
    { id: 'grains', name: 'Крупи та борошно', nameEn: 'Grains & Flour', icon: '🌾', gradient: 'from-amber-600 via-yellow-600 to-orange-500', count: 73 },
    { id: 'preserves', name: 'Консервація', nameEn: 'Preserves', icon: '🫙', gradient: 'from-violet-500 via-purple-500 to-fuchsia-500', count: 89 },
  ]

  // Trust badges - Glassmorphism styled
  const trustBadges = [
    { icon: Truck, title: 'Безкоштовна доставка', titleEn: 'Free Delivery', desc: 'від 500₴', descEn: 'from 500₴', gradient: 'from-emerald-500/20 to-green-500/20' },
    { icon: Shield, title: 'Гарантія якості', titleEn: 'Quality Guarantee', desc: '100% натуральне', descEn: '100% Natural', gradient: 'from-blue-500/20 to-cyan-500/20' },
    { icon: Leaf, title: 'Еко-сертифікація', titleEn: 'Eco-Certified', desc: 'Перевірені ферми', descEn: 'Verified Farms', gradient: 'from-lime-500/20 to-green-500/20' },
    { icon: Award, title: 'DAAR бонуси', titleEn: 'DAAR Rewards', desc: 'Накопичуй токени', descEn: 'Earn Tokens', gradient: 'from-amber-500/20 to-yellow-500/20' },
  ]
  ...
  const heroSlides = [
    {
      title: 'Свіжі продукти з українських ферм',
      titleEn: 'Fresh Products from Ukrainian Farms',
      subtitle: 'Доставка прямо до дверей протягом 24 годин',
    ...
    {
      title: 'Органічні овочі та фрукти',
      titleEn: 'Organic Vegetables & Fruits',
      subtitle: 'Без пестицидів та ГМО — тільки природа',
    ...
    {
      title: 'Знижки до 30% на мед',
      titleEn: 'Up to 30% Off Honey',
      subtitle: 'Тільки цього тижня — встигни замовити!',
          ...
          >
            ...
            <div className="relative z-10 max-w-xl">
              ...
              >
                ...
                {isUkrainian ? 'Нові надходження' : 'New Arrivals'}
              </motion.div>
              ...
              >
                {isUkrainian ? heroSlides[currentSlide].title : heroSlides[currentSlide].titleEn}
              </motion.h1>
              ...
              >
                {isUkrainian ? heroSlides[currentSlide].subtitle : heroSlides[currentSlide].subtitleEn}
              </motion.p>
              ...
              >
                <Link href={ROUTES.STORE(locale)}>
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                  >
                    {isUkrainian ? heroSlides[currentSlide].cta : heroSlides[currentSlide].ctaEn}
          ...
          >
            ...
            <div className="relative z-10 min-w-0 flex-1">
              <div className="font-semibold text-sm truncate text-foreground">
                {isUkrainian ? badge.title : badge.titleEn}
              </div>
              <div className="text-xs text-muted-foreground truncate">
                {isUkrainian ? badge.desc : badge.descEn}
              </div>
            </div>
          </motion.div>
        ))}
      ...
      <motion.section variants={itemVariants} className="px-2 md:px-0">
        <div className="flex items-center justify-between mb-6">
          ...
          >
            ...
            {isUkrainian ? 'Категорії' : 'Categories'}
          </motion.h2>
          <Link href={ROUTES.STORE(locale)}>
            <motion.span 
              ...
              whileHover={{ x: 4 }}
            >
              {isUkrainian ? 'Всі категорії' : 'All Categories'}
        ...
        <motion.section variants={itemVariants} className="px-2 md:px-0">
          <div className="flex items-center justify-between mb-6">
            ...
            >
              ...
              {isUkrainian ? 'Популярні товари' : 'Featured Products'}
            </motion.h2>
            <Link href={ROUTES.STORE(locale)}>
              <motion.span 
                className="text-sm text-primary hover:text-primary/80 flex items-center gap-1 font-medium"
                whileHover={{ x: 4 }}
              >
                {isUkrainian ? 'Переглянути всі' : 'View All'}
                <ChevronRight className="w-4 h-4" />
              </motion.span>
            </Link>
          </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-center md:text-left">
              ...
              >
                ...
                {isUkrainian ? 'Спеціальна пропозиція' : 'Special Offer'}
              </motion.div>
              <h3 className="text-2xl md:text-3xl font-bold text-white mb-3">
                {isUkrainian ? 'Приєднуйтесь до GreenFood!' : 'Join GreenFood!'}
              </h3>
            ...
            <Link href={ROUTES.LOGIN(locale)}>
              ...
              >
                ...
                {isUkrainian ? 'Зареєструватись' : 'Sign Up'}
                <ArrowRight className="w-5 h-5" />
              </motion.button>
            </Link>
          </div>
        ...
        <motion.section variants={itemVariants} className="px-2 md:px-0">
          <div className="flex items-center justify-between mb-6">
            ...
            >
              ...
              {isUkrainian ? 'Вам може сподобатись' : 'You May Like'}
            </motion.h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="relative z-10 flex items-center gap-4">
              ...
              <div>
                <div className="font-bold text-lg">
                  {isUkrainian ? 'Вітаємо' : 'Welcome'}, {session.user?.name || 'User'}! 👋
                </div>

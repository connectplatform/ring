...
import { Users, Globe, Heart, Target, Sparkles, ArrowRight } from 'lucide-react'
import type { LucideIcon } from 'lucide-react'
import type { Locale } from '@/i18n/shared'

type TeamMemberId = 'ray' | 'max' | 'dima'

type ImpactMetric = 'globalDeployments' | 'peopleServed' | 'countriesReached' | 'communitiesEmpowered'

const IMPACT_STAT_CONFIG: ReadonlyArray<{
  icon: LucideIcon
  metric: ImpactMetric
  value: string
  color: string
}> = [
  { icon: Globe, metric: 'globalDeployments', value: '50+', color: 'text-blue-500' },
  { icon: Users, metric: 'peopleServed', value: '10,000+', color: 'text-green-500' },
  { icon: Target, metric: 'countriesReached', value: '25+', color: 'text-purple-500' },
  { icon: Heart, metric: 'communitiesEmpowered', value: '200+', color: 'text-red-500' }
]

const TEAM_MEMBER_ROWS: ReadonlyArray<{ id: TeamMemberId; name: string; image: string }> = [
  { id: 'ray', name: 'Ray', image: '/images/team/ray.jpg' },
  { id: 'max', name: 'Max', image: '/images/team/max.jpg' },
  { id: 'dima', name: 'Dima', image: '/images/team/dima.jpg' }
]
...
const appPublisherPage: React.FC<appPublisherPageProps> = ({
  session: _session,
  locale
}) => {
  const t = useTranslations('pages.home.appPublisher')
      ...
      >
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div variants={itemVariants} className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {IMPACT_STAT_CONFIG.map((stat, index) => (
              <Card key={index} className="text-center">
                <CardContent className="p-6">
                  <stat.icon className={`h-12 w-12 mx-auto mb-4 ${stat.color}`} />
                  <div className="text-3xl font-bold mb-2">{stat.value}</div>
                  <div className="text-sm text-muted-foreground">
                    {t(`impactStats.${stat.metric}`)}
                  </div>
                </CardContent>
              </Card>
            ))}
      ...
      >
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div variants={itemVariants} className="grid md:grid-cols-3 gap-8">
            {TEAM_MEMBER_ROWS.map((member, index) => (
              <Card key={index} className="text-center">
                <CardContent className="p-6">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {t(`teamMembers.${member.id}.specialty`)}
                  </p>
                </CardContent>
              </Card>
            ))}
...
export default appPublisherPage

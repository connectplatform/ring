...
#   npm run build -- ring-n9life-com     # clone overlay: ../ring-n9life-com/web or .dev-merge
#
...
case "$TARGET" in
  ...
  ring-platform-org|org|empire)
    ...
    ;;
  ring-*)
    CLONE="$KINGDOM/$TARGET"
    CLONE_WEB="$CLONE/web"
    test -d "$CLONE" || {
      echo "FATAL: clone not found at $CLONE (kingdom layout — not in connectplatform/ring)" >&2
      exit 1
    }

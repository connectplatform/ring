...
/**
 * Script to add products to the Ring Platform store
 * Uses the POST /api/store/products endpoint
 */
...
async function addProducts() {
  ...
  console.log('Adding products to Ring Platform store...');
  console.log(`Using API endpoint: ${baseUrl}/api/store/products`);
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/store/products`, {
        ...
        headers: {
          ...
          // Note: This will require authentication. In production, you'd need to include auth tokens
        },
      ...
      if (response.ok) {
        ...
        console.log(`✅ Successfully added: ${result.name} (${result.id})`);
      } else {

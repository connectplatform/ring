...
- **Еталонна токеноміка RING** у `contracts/RingToken.sol` та `contracts/Membership.sol`
...
| `MEMBERSHIP_FEE` | 1 RING / місяць | Лише `Membership` викликає `deductMembershipFee` |
...
| Сервіс | Шлях | Роль |
|--------|------|------|
| Кредити | `features/wallet/services/credit-balance-service.ts` | Баланс, списання, членський внесок |
| Підписки | `services/membership/subscription-service.ts` | Створення, скасування, продовження, batch |
| Oracle | `features/wallet/services/native-token-price-oracle.ts` | RING/USD із fallback-джерелами |
...
- `POST /api/membership/payment/token` — оплата RING
...
<Steps>
  ...
  npx hardhat run scripts/deploy-testnet.js --network polygon
    ...
    Порядок: `RingToken` → `Membership` → `setMembershipContract`. JSON деплою — `deployments/mumbai-deployment.json`.
  </Step>
...
1. Форкніть `RingToken.sol` / `Membership.sol` або деплойте як є зі своєю скарбницею
2. Оновіть `constants/web3.ts` та env-адреси
3. Налаштуйте fee constants у `credit-balance-service` та `subscription-service`
...
<Callout type="info">
  **Потрібна допомога з впровадженням?** Перегляньте [можливості кастомізації Ring](/opportunities?type=ring_customization) або [інтеграцію платежів](/docs/customization/payment-integration) для fiat + token checkout.
</Callout>

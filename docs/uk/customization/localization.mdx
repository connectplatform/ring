...
Джерело правди: `lib/locale-config.ts`. Див. [Система локалей](/docs/features/locale-system).
...
export function MyComponent() {
  ...
  return (
    <div>
      ...
      <div>{t('messages.success.saved')}</div>
    </div>
  )
}`}
</Code>
...
export function LocalizedContent() {
  ...
  return (
    <div>
      <div>Локальна валюта: {content[locale].currency}</div>
      <div>Формат дати: {content[locale].dateFormat}</div>
    </div>
  )
}`}
</Code>

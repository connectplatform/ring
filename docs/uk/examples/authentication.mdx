...
description: Шаблони аутентифікації з кількома провайдерами на Auth.js v5 — Google OAuth, Google One Tap, Apple Sign-In, магічні посилання через Resend та аутентифікація через криптогаманець з адаптерами PostgreSQL або Firebase.
...
| **Resend (Magic Links)** | Вхід без пароля через email. Термін дії токена 24 години, одноразове використання. Потребує `AUTH_RESEND_KEY` |
...
<Callout type="tip">
  Auth.js v5 автоматично зчитує `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, `AUTH_APPLE_ID`, `AUTH_APPLE_SECRET` та `AUTH_RESEND_KEY` зі змінних середовища — не потребує ручного налаштування `clientId` / `clientSecret` у конструкторі провайдера.
</Callout>

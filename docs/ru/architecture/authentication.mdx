...
| **Magic Links** | Email через Resend, 24-часовой срок действия токена | `Resend` |
...
Требует `AUTH_RESEND_KEY` переменную окружения и активный адаптер базы данных (для хранения токенов):

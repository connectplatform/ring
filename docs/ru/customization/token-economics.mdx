...
- **Эталонная токеномика RING** в `contracts/RingToken.sol` и `contracts/Membership.sol`
...
| `MEMBERSHIP_FEE` | 1 RING / месяц | Только `Membership` вызывает `deductMembershipFee` |
...
| Сервис | Путь | Роль |
|--------|------|------|
| Кредиты | `features/wallet/services/credit-balance-service.ts` | Баланс, списания, членский взнос |
| Подписки | `services/membership/subscription-service.ts` | Создание, отмена, продление, batch |
| Oracle | `features/wallet/services/native-token-price-oracle.ts` | RING/USD с fallback-источниками |
...
<Steps>
  ...
  npx hardhat run scripts/deploy-testnet.js --network polygon
    ...
    Порядок: `RingToken` → `Membership` → `setMembershipContract`. JSON деплоя — `deployments/mumbai-deployment.json`.
  </Step>
...
1. Форкните `RingToken.sol` / `Membership.sol` или деплойте как есть со своим казначейством
2. Обновите `constants/web3.ts` и env-адреса
3. Настройте fee constants в `credit-balance-service` и `subscription-service`

---
title: Store API
description: Multi-vendor marketplace API with PaymentConductor checkout and internal credit (v1.6.0)
...
Checkout flows through **PaymentConductor** (`store_order` purpose): WayForPay/Stripe redirect, **internal credit** (`POST /api/store/payments/credit`), optional RING token rail. See [PaymentConductor](/docs/library/features/payment-conductor).
</Callout>
...
```
Product Listing → Order Creation → Payment Processing → Fulfillment → Settlement
                                      ↓
          ...
          <UiCard key={product.id} className="product-card">
            ...
            <UiCardContent>
              ...
              <div className="vendor">
                by {product.vendor.name} ({product.vendor.trustScore}⭐)
              </div>

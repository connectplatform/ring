...
`settlement.ts` calculates platform commission, optional **referral commission** (when order attribution exists), and custom splits from merchant configuration. This is the **vendor-funded rail**; platform-funded **token** rewards are handled by [refcodes](/docs/library/features/refcodes).

<Callout type="info" title="Dual-rail">
  Same `orderReference` should appear on `settlements`, `erp_sales_assists`, and `referral_rewards` for cross-audit. See [Affiliate enablement](/docs/library/features/affiliate-enablement).
</Callout>
...
- [Affiliate enablement](/docs/library/features/affiliate-enablement) — dual-rail overview and verification
- [Referral codes](/docs/library/features/refcodes) — shared commission hierarchy and token rewards
- [Vendor management](/docs/library/features/erp/vendor-management)
- [Inventory](/docs/library/features/erp/inventory)

...
Store checkout flows through **PaymentConductor** (fiat) or **RING credit / on-chain** subscription (Web3-enabled clones). Settlement, vendor referral commission, and platform token rewards are documented under [PaymentConductor](/docs/architecture/payment-conductor), [Affiliate enablement](/docs/library/features/affiliate-enablement), and [ERP commissions](/docs/library/features/erp/commissions).

### Content + locale

Public pages use **next-intl** with shared label modules. News and docs content are **per-locale files** under `docs/content/{locale}/library/`. EN is canonical unless LOCALE-GAPS marks a condensed UK/RU summary.

...
**Operator setup:** [Payment integration](/docs/library/customization/payment-integration) · [Environment — PaymentConductor](/docs/library/deployment/environment#payment-conductor)
...
```typescript
// lib/payments/conductor/types.ts (summary)
type PaymentPurpose =
  | 'store_order'
  | 'membership_upgrade'
  | 'news_promotion'
...
`dispatchWayForPayWebhook(payload)`:

1. Read `orderReference` from payload
2. `parseOrderReference` → purpose
3. **Store:** `verifyWayForPayStoreWebhook` → `handleStoreWayForPayWebhook`
4. **Membership / news:** `verifyWayForPayGenericWebhook` → respective handler
5. Membership success returns WayForPay ACK (`buildMembershipWebhookAck`)

`dispatchStripeWebhook(rawBody, signature)`:

1. `verifyStripeWebhook`
2. Read `metadata.purpose` (default `news_promotion`)
3. `handleNewsStripeWebhook` on `checkout.session.completed` / news purpose

Canonical routes call the dispatcher directly:

- `app/api/payments/wayforpay/webhook/route.ts`
- `app/api/payments/stripe/webhook/route.ts`

## API routes

### Canonical (register these in PSP dashboards)

| Method | Path | Role |
|--------|------|------|
| `POST` | `/api/payments/wayforpay/webhook` | Unified WayForPay callback |
| `POST` | `/api/payments/stripe/webhook` | Stripe signed events |
| `POST` | `/api/store/payments/wayforpay` | Initiate store WayForPay redirect |
| `GET` | `/api/store/payments/[orderId]/status` | Poll order payment status |
| `POST` | `/api/store/payments/credit` | Store checkout via internal credit |
| `POST` | `/api/membership/payment/ring` | RING token membership payment |
| `POST` | `/api/news/promotion/submit` | Start news promotion checkout |

### Deprecated aliases (delegate to dispatcher)
...
`app/api/membership/payment/credit/route.ts` re-exports `membership/payment/ring` — credit rail for membership uses RING/wallet paths, not a separate credit endpoint.
...
- [Payments feature overview](/docs/library/features/payments)
- [Payment integration (operator)](/docs/library/customization/payment-integration)

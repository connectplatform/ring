...
Ring Platform requires **`DB_BACKEND_MODE`** at runtime (`k8s-postgres-fcm` | `firebase-full` | `supabase-fcm`). It controls PostgreSQL vs Firebase for **application data** and how **FCM** is wired. **`DB_HYBRID_MODE`** is a separate legacy toggle — see [Backend modes and databases](/en/library/architecture/backend-modes-and-databases).

Optional **Tunnel** realtime may use Supabase (`NEXT_PUBLIC_TUNNEL_TRANSPORT=supabase`) regardless of `DB_BACKEND_MODE`; see [Tunnel protocol](/en/library/features/tunnel-protocol).
...
See `lib/locale-config.ts` and [Locale system](/docs/library/features/locale-system).
...
| Block | Purpose | Doc |
|-------|---------|-----|
| `IMAGE_GEN_*`, `XAI_IMAGE_*` | ImageConductor, `ring-image-create` | [generative-images](/en/library/development/generative-images) |
| `TEXT_GEN_*`, `XAI_TEXT_*` | TextConductor, cited articles | [generative-newsroom](/en/library/development/generative-newsroom) |
| `TTS_*`, `XAI_TTS_*` | AudioConductor narration | [generative-newsroom](/en/library/development/generative-newsroom) |
| `NEWS_AUTOGEN_*` | Draft defaults (visibility, audio) | [generative-newsroom](/en/library/development/generative-newsroom) |
...
See [Email AI-CRM feature](/docs/library/features/email-ai-crm), [API](/docs/library/api/email-ai-crm), and [operator guide](/docs/library/examples/email-ai-crm).

...
<Callout type="info">
  **Canonical env model:** Ring Platform selects the primary database and push stack with **`DB_BACKEND_MODE`** (`k8s-postgres-fcm`, `firebase-full`, `supabase-fcm`). Postgres‑primary production still uses Firebase for **FCM** when enabled — not Firestore as the system of record. Full matrix: [Backend modes and databases](/en/library/architecture/backend-modes-and-databases).
</Callout>
...
- [Reference deployments](success-stories) - Named Ringdom clones and live links

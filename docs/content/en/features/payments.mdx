...
description: PaymentConductor with WayForPay, Stripe, internal credit — store, membership, and news promotion
last_modified: "2026-06-06"
---

# Payment Integration

Ring Platform **v1.6.0** routes all card and credit flows through **PaymentConductor** (`lib/payments/conductor/`). WayForPay remains the primary PSP for Ukraine; Stripe is supported for international rails.

> **Deep dive:** [PaymentConductor feature](/docs/library/features/payment-conductor) · [Architecture](/docs/library/architecture/payment-conductor)

## Overview

PaymentConductor supports:

- **Store checkout** — `store_order` purpose, WayForPay/Stripe redirect or internal credit
- **Membership upgrades** — `membership_upgrade` purpose
- **News promotion** — `news_promotion` purpose (News Kingdom)
- **Ledger** — `payment_transactions` table (migration 004)
- **Webhooks** — Unified dispatcher for WayForPay and Stripe

Legacy WayForPay-only flows below remain accurate for merchant configuration; orchestration is now conductor-driven.
...
- 🏦 **Banking**: Privat24, LiqPay
- 📅 **Installments**: PayParts, instant installments
  ...
  return (
    <div className="modal">
      ...
      <div className="benefits">
        <h3>Benefits:</h3>
        <ul>
          <li>✅ Create and manage entities</li>

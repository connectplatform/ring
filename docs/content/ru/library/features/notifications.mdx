...
- **Push Notifications (FCM)** — Веб- и мобильные push через Firebase Cloud Messaging: полная настройка, регистрация токенов через Server Action и контракт API — см. [Push-уведомления через FCM (Ring)](/library/features/push-notifications-fcm).
...
Полное руководство по настройке FCM — регистрация токенов через Server Action, ограниченный по частоте API для клиентов без React, переменные окружения и устранение неполадок:

**[Push-уведомления через FCM (Ring)](/library/features/push-notifications-fcm)**

---

*Полная документация по уведомлениям (триггеры, API настроек) готовится.*

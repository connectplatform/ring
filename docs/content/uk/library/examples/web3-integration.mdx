...
description: Документація інтеграції Web3 для платформи Ring
---

# Приклади інтеграції Web3

Повні приклади інтеграції Web3 для блокчейн-функцій платформи Ring, включаючи гаманці, NFT та стейкінг токенів.
...
export function WalletDashboard() {
  ...
  // Токени платформи Ring
  const supportedTokens = [
    { symbol: 'ETH', address: '', logo: '⚡' },
    { symbol: 'RING', address: process.env.NEXT_PUBLIC_RING_TOKEN_ADDRESS, logo: '🔗' },
    { symbol: 'DAAR', address: process.env.NEXT_PUBLIC_DAAR_TOKEN_ADDRESS, logo: '💎' },
    { symbol: 'DAARION', address: process.env.NEXT_PUBLIC_DAARION_TOKEN_ADDRESS, logo: '👑' },
  ]
  ...
  const loadWallet = async () => {
    try {
      ...
      console.error('Не вдалося завантажити гаманець:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const createWallet = async () => {
    ...
    try {
      ...
      console.error('Не вдалося створити гаманець:', error)
    } finally {
      setCreating(false)
    }
  }
  ...
  if (loading) {
    return <div className="p-4">Завантаження гаманця...</div>
  }

  if (!wallet) {
    return (
      <div className="max-w-md mx-auto p-6 text-center">
        <h2 className="text-2xl font-bold mb-4">Створіть свій гаманець</h2>
        <p className="text-gray-600 mb-6">
          Створіть безпечний гаманець для участі у Web3 функціях платформи Ring.
        </p>
        <button
        ...
        >
          {creating ? 'Створення гаманця...' : 'Створити гаманець'}
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg p-6 mb-6">
        <h2 className="text-2xl font-bold mb-2">Ваш гаманець</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <button className="bg-green-600 text-white p-4 rounded-lg hover:bg-green-700">
          <div className="text-lg font-semibold">Надіслати токени</div>
          <div className="text-sm opacity-90">Перевести в інший гаманець</div>
        </button>
        
        <button className="bg-blue-600 text-white p-4 rounded-lg hover:bg-blue-700">
          <div className="text-lg font-semibold">Застейкати токени</div>
          <div className="text-sm opacity-90">Отримувати винагороди за стейкінг</div>
        </button>
        
        <button className="bg-purple-600 text-white p-4 rounded-lg hover:bg-purple-700">
          <div className="text-lg font-semibold">NFT Маркетплейс</div>
          <div className="text-sm opacity-90">Купувати та продавати NFT</div>
        </button>
      </div>
    </div>
  )
...
export function StakingDashboard() {
  ...
  const loadStakingPools = async () => {
    try {
      ...
      console.error('Не вдалося завантажити пули стейкінгу:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const stakeTokens = async (poolId: string, amount: string) => {
    ...
    try {
      ...
      if (response.ok) {
        loadStakingPools()
        alert('Токени успішно застейкано!')
      } else {
        throw new Error('Стейкінг не вдався')
      }
    } catch (error) {
      console.error('Не вдалося застейкати токени:', error)
      alert('Не вдалося застейкати токени')
    } finally {
      setStaking(null)
    }
  }
  ...
  const unstakeTokens = async (poolId: string) => {
    try {
      ...
      if (response.ok) {
        loadStakingPools()
        alert('Токени успішно знято зі стейкінгу!')
      } else {
        throw new Error('Зняття зі стейкінгу не вдалося')
      }
    } catch (error) {
      console.error('Не вдалося зняти токени зі стейкінгу:', error)
      alert('Не вдалося зняти токени зі стейкінгу')
    }
  }
  ...
  const claimRewards = async (poolId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        throw new Error('Отримання винагород не вдалося')
      }
    } catch (error) {
      console.error('Не вдалося отримати винагороди:', error)
      alert('Не вдалося отримати винагороди')
    }
  }

  if (loading) {
    return <div className="p-4">Завантаження пулів стейкінгу...</div>
  }

  return (
    <div className="max-w-4xl mx-auto p-6">
      <h2 className="text-3xl font-bold mb-6">Стейкінг токенів</h2>
          ...
          <div key={pool.id} className="bg-white border rounded-lg p-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-gray-600">Пул стейкінгу {pool.token}</p>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-green-600">{pool.apr}%</div>
                <div className="text-sm text-gray-600">Річна дохідність</div>
              </div>
            </div>

            <div className="space-y-3 mb-6">
              <div className="flex justify-between">
                <span className="text-gray-600">Загалом застейкано:</span>
                <span className="font-medium">{pool.totalStaked} {pool.token}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Ваш стейк:</span>
                <span className="font-medium">{pool.userStaked} {pool.token}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Очікувані винагороди:</span>
                <span className="font-medium text-green-600">{pool.rewards} {pool.token}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Період блокування:</span>
                <span className="font-medium">{pool.lockPeriod} днів</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Мін. стейк:</span>
            ...
            <div className="space-y-3">
              {parseFloat(pool.userStaked) === 0 ? (
                <button
                  onClick={() => {
                    const amount = prompt(`Введіть суму для стейкінгу (мін. ${pool.minStake} ${pool.token}):`)
                ...
                >
                  {staking === pool.id ? 'Стейкінг...' : 'Застейкати токени'}
                </button>
              ) : (
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => {
                      const amount = prompt(`Введіть додаткову суму для стейкінгу:`)
              ...
              {parseFloat(pool.rewards) > 0 && (
                ...
                >
                  Отримати винагороди ({pool.rewards} {pool.token})
                </button>
              )}
            </div>
          </div>
      ...
      {pools.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500">Пули стейкінгу недоступні.</p>
        </div>
      )}
    </div>
  )
...
### Сервіс взаємодії з контрактами
...
*Готові створювати повноцінні додатки? Перевірте [Реальні додатки](/uk/docs/examples/real-world) або дослідіть [Приклади White Label](/uk/docs/examples/white-label).*

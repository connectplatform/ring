...
Ring’s background health (email CRM poll, reservation cleanup, subscription expiry, settlements, and more) runs through **ProcessConductor** — currently **19** pipelines in `lib/processes/registry.ts`. Each pipeline exposes `GET/POST /api/cron/<path>` gated by `Authorization: Bearer $CRON_SECRET`.
...
<Callout type="warning">
  Deploying Hosting alone does **not** schedule ProcessConductor jobs. Wire Cloud Scheduler to the same cron URLs using the [Vercel schedule catalog](/docs/deployment/vercel), and set `CRON_SECRET` in the app env.
</Callout>
...
3. Create one Cloud Scheduler job per entry (UTC schedule → `GET` or `POST` your public `/api/cron/...` URL with `Authorization: Bearer <CRON_SECRET>`).
...
| Routes | `app/api/cron/<name>/route.ts` — `GET`/`POST`, fail-closed on `Authorization: Bearer $CRON_SECRET` |
...
Copy every `{ path, schedule }` from root `vercel.json` `crons[]`. Full catalog (UTC) is also documented on [Vercel deployment](/docs/deployment/vercel).
...
- **Target:** `https://<your-ring-host><path>` (e.g. `/api/cron/email-processor`)
...
curl -sS -H "Authorization: Bearer $CRON_SECRET" \
  "https://<your-ring-host>/api/cron/cleanup-usernames"
```
...
`isFcmConfigured()` / `validateFcmVapidKey()` in `lib/firebase-client.ts` require a non-placeholder `NEXT_PUBLIC_FIREBASE_VAPID_KEY`. Cross-project ring-main certificate leak detection: `isKnownCrossProjectVapidLeak()`. Deep push map: [Push notifications (FCM)](/docs/features/push-notifications-fcm).
...
<RelatedDocs>
  <RelatedArticle
    ...
    relation="Same-workflow: vercel.json is the ProcessConductor schedule catalog — copy crons[] into Cloud Scheduler."
  />
  <RelatedArticle
    slug="architecture/backend-modes-and-databases"
    relation="Prerequisite: firebase-full vs k8s-postgres-fcm vs supabase-fcm."
  />
  <RelatedArticle
    slug="features/push-notifications-fcm"
    relation="Next-step: FCM token registration and Server Action upsert."
  />
  <RelatedArticle
    slug="deployment/environment"
    relation="Depends-on: AUTH_FIREBASE_*, NEXT_PUBLIC_FIREBASE_*, and CRON_SECRET."
  />
  <RelatedArticle
    slug="architecture/authentication"
    relation="See-also: Auth.js v5 — Firebase Auth is not used."
  />
  <RelatedArticle
    slug="backend/k8s-postgres-fcm"
    relation="See-also: PostgreSQL-primary mode with FCM-only Firebase."
  />
  <RelatedArticle
    slug="features/admin"
    relation="Next-step: Admin → Background Processes labels for pipeline runs."
  />
  <RelatedArticle
    slug="features/email-ai-crm"
    relation="Same-workflow: email-processor / email-analytics cron pipelines."
  />
</RelatedDocs>

...
Ring's user-facing REST routes use **Auth.js session cookies**. The MCP gateway validates `Authorization: Bearer` against `RING_MCP_ACCESS_KEY` and injects a service actor — see [Ring MCP Server — Architecture](/docs/development/ring-mcp#architecture-notes).

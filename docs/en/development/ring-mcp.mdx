...
| **Per-clone tenancy** | One `ring-mcp` entry per clone via `RING_API_BASE_URL` + `RING_MCP_ACCESS_KEY` |
...
**Full reference:** [Autonomous newsroom (generative-newsroom)](/en/docs/development/generative-newsroom) — parameters, Telegram flow, translation fan-out, env block, troubleshooting.
...
**Full reference:** [Generative images (generative-images)](/en/docs/development/generative-images) — providers, OG presets, env block, `GenerateImageDialog`, migration `006_generated_images_schema.sql`.
...
**Full reference:** [Generative videos (generative-videos)](/en/docs/development/generative-videos) — draft vs production, CLI manifest, remaster flow, env block.
...
1. `Authorization: Bearer <token>` checked against `RING_MCP_ACCESS_KEY`
...
- Check `RING_MCP_ACCESS_KEY` in env.
- Restart Ring after env changes
- No `Bearer` prefix in env — the client adds it

### `RING_MCP_ACCESS_KEY is not configured`

- Set the variable on the **Ring app** env (.env.local and k8s secrets), not only in MCP config
...
### `RING_MCP_ACCESS_KEY is not set`

- Fill `RING_MCP_ACCESS_KEY` in `.cursor/mcp.json` and reload MCP

...
<Callout type="info">
  Use **Founder** / **Developer** tabs in the docs sidebar to filter this page. See [Authentication Architecture](/docs/architecture/authentication) for provider design decisions and [Environment Variables](/docs/deployment/environment) for the full env reference.
</Callout>
...
Ring wraps Auth.js with a tuned `SessionProvider` at `features/auth/components/session-provider.tsx`. Import this component — not `SessionProvider` directly from `next-auth/react` — so refetch behavior is consistent app-wide:
...
The canonical implementation sets `refetchInterval={15 * 60}` (one 15-minute background poll), `refetchOnWindowFocus={false}`, and `refetchWhenOffline={false}` — reducing redundant `GET /api/auth/session` calls across every `useSession()` consumer.
...
<Cards>
  <Card href="/docs/architecture/authentication" title="Authentication Architecture" description="Provider design, session flow, and file structure." />
  <Card href="/docs/deployment/environment" title="Environment Variables" description="Complete env var reference including Auth.js and all providers." />

...
Set up the Email AI-CRM module on a **local or self-hosted** Ring clone: configure `.env.local`, apply JSONB migrations when using Postgres, then verify ingest and draft send from the admin CRM UI (`/admin/crm/*`).
...
3. Open **Admin → CRM Inbox** (`/admin/crm/inbox`) — thread should appear.
4. Open **CRM Drafts** (`/admin/crm/drafts`) — approve a draft, click send.
...
| Page | URL | Expect |
|------|-----|--------|
| Inbox | `/{locale}/admin/crm/inbox` | Live threads from API |
...
All seven CRM pages use `CrmAdminShell` (horizontal tabs; no right rail). APIs remain `/api/admin/email/*`.

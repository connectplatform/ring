...
Hands-on patterns for building on Ring Platform. The platform exposes **~244 App Router `/api` route handlers** (see [API Reference](/docs/api) and the RingApiTree widget); this page shows the integration paths developers actually use in alpha clones.
...
| **MCP / external** | Automation against `/api/mcp/v1/*` with API keys — see [API Reference](/docs/api) |
</Callout>
...
Full endpoint lists: [Entities](/docs/api/entities), [Opportunities](/docs/api/opportunities), [Messaging](/docs/api/messaging), [Notifications](/docs/api/notifications).
...
Responses use **`SerializedEntity`** (ISO date strings). See [Entities API](/docs/api/entities) for the full shape and role-based visibility.
...
After mutations, list freshness is driven by `revalidatePath` + Tunnel `entity:*` events ([Discovery mutation sync](/docs/architecture/discovery-mutation-sync)).

---

## Opportunities

**Routes:** `GET /api/opportunities` · `GET /api/opportunities/{id}` · create/update via Server Actions or documented POST routes in [Opportunities API](/docs/api/opportunities).
...
Persist messages through REST; receive live updates on `conversation:${id}` via Tunnel ([Messaging API](/docs/api/messaging), [Tunnel protocol](/docs/features/tunnel-protocol)).
...
Server-side, new notifications call `publishToUserTunnel(userId, 'notifications:unread', { count })` — see [Notifications API](/docs/api/notifications).
...
- [API Reference](/docs/api) — full route index
- [Authentication](/docs/api/authentication) — Auth.js providers and session
- [Tunnel protocol](/docs/features/tunnel-protocol) — SSE, poll, env vars
- [Web3 integration](/docs/examples/web3-integration) — wallet and on-chain flows
- [Real-world apps](/docs/examples/real-world) — end-to-end clone patterns

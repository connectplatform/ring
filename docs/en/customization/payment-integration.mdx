...
description: Operator guide — PaymentConductor, WayForPay, Stripe, and internal credit for store, membership, news promotion, and wallet top-up
...
**Processors:** WayForPay (default), Stripe, internal credit. PayPal membership is **not** implemented (FutureFeature pool `membership-paypal`).
...
<FutureFeatureWidget
  ...
  description="Operator-facing PayPal merchant setup for membership. UI tab exists; processor and cabinet checklist not shipped (poolSlug membership-paypal)."
  implementationCost={96}
  labels={['payments', 'paypal', 'ops']}

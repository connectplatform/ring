...
**Desk oracle (shared):** Token Desk credit↔native conversion and public-pool card settle both use `getNativeTokenPerMainCurrencyRate()` (`nativeUi = fiatMajor / nativePerMainCurrency`). Do **not** confuse with `wallet_topup` (fiat credit points, 1:1 ledger). Full jar SSOT: [Public Pools & DAO Jars](/docs/features/public-pools).

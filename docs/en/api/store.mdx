...
- **Credit vs card** — credit for UAH carts needs `PAYMENT_CREDIT_ACCEPT_ORDER_CURRENCY=UAH` (or your accepted list); otherwise buyers use card.
...
<Step>
Ensure `PAYMENT_STORE_ALLOW_CREDIT` is not `false` and UAH (if used) is listed in `PAYMENT_CREDIT_ACCEPT_ORDER_CURRENCY`.
</Step>

<Step>
`POST /api/store/payments/credit` with `{ orderId }` — conductor rail `internal_credit`; on success the route marks the order paid and runs stock deduct + settlements.
</Step>
...
<Cards>
  ...
  <Card title="Wallet API" href="/docs/api/wallet">
    Credit balance for internal_credit rail.
  </Card>

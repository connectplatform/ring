...
Message `type` values in chat types: `text` | `image` | `file` | `system` | `payment_request`. Message `metadata.kind` (e.g. `payment_request`, `gallery_upload`) is a **different** namespace from conversation `metadata.kind`.

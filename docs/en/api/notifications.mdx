...
<Callout type="warning" title="Firebase certificate vs staged VAPID_*">
  Browser `getToken()` needs `NEXT_PUBLIC_FIREBASE_VAPID_KEY` — the Firebase Console → Cloud Messaging → Web Push certificates public key for this clone’s Firebase project (confirmed for **ring-platform**). Staged `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` are for planned `web-push` dual-stack and are **not** read by Ring code yet. Full map: [Push notifications (FCM)](/docs/features/push-notifications-fcm).
</Callout>
...
<Callout type="info" title="Staged RFC Web Push (Developer)">
  Dual-stack prep stores `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` in secrets/env. No `web-push` readers yet — see [Push notifications (FCM)](/docs/features/push-notifications-fcm). Rebuild the image whenever you rotate `NEXT_PUBLIC_FIREBASE_VAPID_KEY`.
</Callout>

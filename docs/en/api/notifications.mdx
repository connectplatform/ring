...
<Callout type="info" title="Real-time delivery">
Unread counts and in-app alerts use **Tunnel Protocol** — server `publishToUserTunnel` on channel `notifications:unread`, client `useSync` via `TunnelProvider`. FCM handles push when the tab is closed.
</Callout>
...
<Code language="json" title="Register body">
{`{
...
<Callout type="info" title="Server-derived user">
...
<Callout type="warning" title="VAPID: Firebase certificate ≠ RFC web-push keys">
  Browser `getToken()` needs `NEXT_PUBLIC_FIREBASE_VAPID_KEY` (Firebase Console → Cloud Messaging → Web Push certificates). That is **not** the planned RFC Web Push trio (`VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` for the `web-push` library — **not shipped**; package not in `package.json`). Do not copy the Firebase certificate into the RFC vars. Full split: [Push notifications (FCM)](/docs/features/push-notifications-fcm).
</Callout>

<Callout type="tip" title="Register path">
...
<Code language="typescript" title="useFCM + useAuth">
...
`public/firebase-messaging-sw.js` — Firebase **12.x** compat scripts for background messages.

<Code language="javascript" title="firebase-messaging-sw.js (excerpt)">
...
See [Push notifications (FCM)](/docs/features/push-notifications-fcm) for the full lifecycle, migration `016_fcm_jsonb_schema.sql`, and troubleshooting.
...
**Shipped FCM** — there are no `FCM_SERVER_KEY` or `FCM_VAPID_KEY` variables. Use Firebase client + Admin env from `env.local.template`:

<Code language="bash" title="FCM client + Admin (shipped)">
...
NEXT_PUBLIC_FIREBASE_VAPID_KEY=…   # NOT an RFC web-push keypair

# Server — Firebase Admin SDK send (not a legacy FCM server key)
...
<Callout type="info" title="Planned RFC Web Push (Developer)">
  Dual-stack browser push will add app-owned keys for the `web-push` library: `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` (generate with `npx web-push generate-vapid-keys`). These are **separate** from `NEXT_PUBLIC_FIREBASE_VAPID_KEY`. Not implemented yet — see commented block in `env.local.template` and [Push notifications (FCM)](/docs/features/push-notifications-fcm).
</Callout>

...
Ring Platform integrates with Firebase Cloud Messaging. Browser registration SSOT: `hooks/use-fcm.ts` inside `FCMProvider`, persisting via `upsertFcmToken` Server Action. Logout SSOT: `useAuth().signOut()` unregisters the device token.

<Callout type="tip" title="Primary path">
Do not `fetch('/api/notifications/fcm/register')` from React — use the Server Action. Reserve the API route for mobile and external clients.
</Callout>

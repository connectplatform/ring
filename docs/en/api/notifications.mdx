...
`NotificationProvider` uses `useUnreadCount` → `useSync` on channel `notifications:unread`. When the tunnel is connected, the badge updates without waiting for the poll interval.

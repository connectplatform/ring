...
Uploads use the resolved `file()` provider (`local_storage`, `vercel_blob`, or `ring_filebase` — see [RingFileBase](/docs/integrations/ring-filebase)). Object bytes are **not** inside Postgres. Use your provider’s export/replication (MinIO/`mc mirror`, Blob export) or treat uploads as rebuildable only if you accept media loss.

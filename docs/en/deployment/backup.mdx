...
Uploads use Vercel Blob when `BLOB_READ_WRITE_TOKEN` is set (`docker-compose.yml`, `env.local.template`). Blob objects are **not** inside Postgres. Use your provider’s export/replication or treat uploads as rebuildable only if you accept media loss.

...
<Code language="bash" title="terminal">
{`docker push registry.ringdom.org/ringdom/ring:v0.9.18-ring-platform.org-amd64`}
</Code>
...
{`deployment/ring-platform \
  ring-platform=registry.ringdom.org/ringdom/ring:v0.9.18-ring-platform.org-amd64'`}
</Code>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: ring-platform
        image: registry.ringdom.org/ringdom/ring:v0.9.18-ring-platform.org-amd64
        ports:
        - containerPort: 3000
          name: http
        env:
...
Code blocks are highlighted **once on the server** (`highlightCodeToHtml` + cached Shiki highlighter). Fenced markdown is converted to `<Code>` via `rehypeCodeFenceToMdx` — there is no client-side Shiki pass.
...
<Code language="bash" title="terminal">
{`kubectl set image deployment/ring-platform \
  ring-platform=registry.ringdom.org/ringdom/ring:v0.9.19-ring-platform.org-amd64 \
  -n ring-platform-org
...
- **v1.6.2**: Unified server-only Shiki (`rehypeCodeFenceToMdx` → async `<Code>`); removed client re-highlight and `@shikijs/rehype`

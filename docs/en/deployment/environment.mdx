...
## Database and push backend modes

Ring Platform requires **`DB_BACKEND_MODE`** at runtime (`k8s-postgres-fcm` | `firebase-full` | `supabase-fcm`). It controls PostgreSQL vs Firebase for **application data** and how **FCM** is wired. **`DB_HYBRID_MODE`** is a separate legacy toggle — see [Backend modes and databases](/en/docs/architecture/backend-modes-and-databases).

Optional **Tunnel** realtime: driven by `RING_DEPLOY_TARGET` (`vercel` = SSE + poll; `k8s` / `self-hosted` = native WSS at `/api/tunnel/ws` + fallback). Set `NEXT_PUBLIC_TUNNEL_TRANSPORT=supabase` for Supabase Realtime when URL/anon key are present. Orthogonal to `DB_BACKEND_MODE` — see [Tunnel protocol](/en/docs/features/tunnel-protocol).
...
See `lib/locale-config.ts` and [Locale system](/docs/features/locale-system).

## PaymentConductor (v1.6.0) {#payment-conductor}
...
## Generative conductors (xAI Grok, 2026-06)

Shared credential: `XAI_API_KEY` + `XAI_API_BASE_URL=https://api.x.ai/v1`.

| Block | Purpose | Doc |
|-------|---------|-----|
| `IMAGE_GEN_*`, `XAI_IMAGE_*` | ImageConductor, `ring-image-create` | [generative-images](/en/docs/development/generative-images) |
| `TEXT_GEN_*`, `XAI_TEXT_*` | TextConductor, cited articles | [generative-newsroom](/en/docs/development/generative-newsroom) |
| `TTS_*`, `XAI_TTS_*` | AudioConductor narration | [generative-newsroom](/en/docs/development/generative-newsroom) |
| `NEWS_AUTOGEN_*` | Draft defaults (visibility, audio) | [generative-newsroom](/en/docs/development/generative-newsroom) |

Requires ring-filebase (`FILE_BACKEND=ringbase`, `RINGBASE_API_URL`, `RINGBASE_API_TOKEN`) for CDN uploads.

**Never commit real secrets** — use `.env.local` (gitignored) and placeholders in docs only.

## Email AI-CRM {#email-ai-crm}

Configure in **`.env.local`** (full commented block in `env.local.template`):

```bash
# .env.local — community contact inbox
...
EMAIL_BATCH_SIZE=10
EMAIL_CRM_PERSISTENCE=memory    # omit when using Postgres migrations 009/010
EMAIL_AUTO_SEND_ENABLED=false
EMAIL_PROCESSOR_AUTOSTART=false # prefer cron poll in production

CRON_SECRET=generate-a-long-random-string
WEBHOOK_EMAIL_SECRET=           # POST /api/webhooks/email/inbound
SLACK_WEBHOOK_URL=              # optional security/urgent alerts
```

Apply `009_email_crm_jsonb.sql` and `010_email_crm_tasks_jsonb.sql` before Postgres-backed ingest. Production: `POST /api/cron/email-processor` with `{"action":"poll"}` every 1–5 minutes.

See [Email AI-CRM feature](/docs/features/email-ai-crm), [API](/docs/api/email-ai-crm), and [operator guide](/docs/examples/email-ai-crm).
...
- [Deployment Guide](/en/docs/deployment/docker) - Container deployment
- [Security Architecture](/en/docs/features/security) - Security best practices
- [Backup & Recovery](/en/docs/deployment/backup) - Data protection
- [Monitoring Guide](/en/docs/deployment/monitoring) - System monitoring

...
description: Required and optional environment variables for Ring Platform — database backend, Firebase Admin SDK, FCM, payments, locales, Tunnel.
...
Requires `FILE_BACKEND=ringbase` with `RINGBASE_API_URL` and `RINGBASE_API_TOKEN` for CDN uploads.

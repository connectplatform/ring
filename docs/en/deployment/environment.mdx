...
Get client SDK keys from **Firebase Console → Project Settings → General → Web apps**. Set `NEXT_PUBLIC_FIREBASE_VAPID_KEY` from **Cloud Messaging → Web Push certificates** for that same project (build-time for the browser). Staged dual-stack: `VAPID_*` — see [Push notifications (FCM)](/docs/features/push-notifications-fcm).
...
Auth.js v5 reads `AUTH_*` prefixed vars for OAuth/OIDC providers (Google, Telegram, Apple). Email auth uses Ring Mailer — see [Ring Mailer](/docs/features/ring-mailer) and [Authentication](/docs/features/authentication).

...
Advanced performance optimization strategies for React 19 + Next.js 16 applications with Firebase integration, achieving 95% Firebase call reduction and 30-40% build time improvement.
...
{`class DatabaseService {
  // React 19 cache() enabled operations (illustrative — production uses lib/database/DatabaseService.ts + BackendSelector)
  read = cache(async (collection: string, id: string) => {
    // Production routing: DB_BACKEND_MODE → initializeDatabase() registers PostgreSQLAdapter and/or FirebaseAdapter
  ...
  private getBackendForCollection(collection: string) {
    // DB_HYBRID_MODE is a legacy per-feature toggle (store, notifications); do not confuse with DB_BACKEND_MODE.
    // Pseudocode below illustrates a per-collection split; real routing: lib/database/BackendSelector.ts
    const postgresOnly = process.env.DB_HYBRID_MODE === 'false'

    if (postgresOnly) {
      return this.postgresBackend // PostgreSQL primary
    }

    // Illustrative split during migration-style rollout

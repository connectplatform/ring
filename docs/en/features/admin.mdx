...
<Cards>
  ...
  <Card title="Email AI-CRM" href="/docs/features/email-ai-crm">
    Inbox, drafts, contacts, analytics, and tasks share the admin shell and email rail.
  </Card>
...
3. Use the **right rail** on News, Store, Email, Matcher, DAO, Web3, Users, or Moderation for in-module links (and related hubs).
...
| Email shell | `features/admin/email/email-admin-shell.tsx` |
| Routes | `constants/routes.ts` (`ADMIN_*`) |
...
| `email` | `navGroupEmail` | Email inbox | `admin` |
...
| `email-*` | `email` | Inbox, drafts, contacts, analytics, tasks |
...
All email CRM routes wrap with `EmailAdminShell` + correct `pageContext` (`email-inbox`, `email-drafts`, …) so the rail submenu works on mobile and desktop.
...
<Cards>
  <Card title="Admin API" href="/docs/api/admin">
    Verified `/api/admin/*` routes and access tiers.
  </Card>
  <Card title="Architecture — security" href="/docs/architecture/security">
    RBAC layers and confidential tiers.
  </Card>
  <Card title="Email AI-CRM" href="/docs/features/email-ai-crm">
    Inbox CRM admin UI and ingest.
  </Card>
  <Card title="Whitelabel navigation" href="/docs/development/whitelabel-navigation">
    Clone-level nav customization patterns.
  </Card>
</Cards>

...
Shared shell: `OrderLabPageShell` → `RingRightRailLayout` + `DavinciCenterPane` + `OrderLabNavRail` (active = beam chrome; status via StatusDot) + `OrderLabHeroStats`. Deep-links: `?tab=<id>`, `?source=<path>` (opens Source tab), `#project-config` / `#secrets`. Status SSOT: `computeOrderLabTabStatuses` (server first paint) + live client bumps (`refreshHero`, `tabStatusFromDeploySnapshot`, `tabStatusFromSourceTree` — empty/scaffold → orange incomplete). `GET /api/my-jobs/[id]/deployment/status` (server-side clone `/api/health` — no browser CORS). Client fetches use `fetchJsonSafe`. Manage client lives at `features/crm/orders/crm-order-detail-client.tsx`.

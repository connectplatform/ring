...
# RING_MCP_ACCESS_KEY=dev-only-change-me`}</Code>

<Callout type="warning" title="Security">
Never commit `XAI_API_KEY` or `RING_MCP_ACCESS_KEY` to the repository. MCP routes are bearer-gated via `withMcpGuard`.
</Callout>

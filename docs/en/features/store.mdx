...
<Callout type="tip">
  Storefront **product agent** chats use `Conversation.type: 'product'` and stay visible in Messages. **Generative Gallery** tool chats reuse the same channel with `kind: 'generative_gallery'` and `hiddenFromInbox: true` so generation history does not clutter the inbox — see [Messaging](/docs/features/messaging) and [Generative Gallery](/docs/features/generative-media).
</Callout>
...
<Cards>
  <Card title="Generative Gallery" href="/docs/features/generative-media">
    Upload \| Generate product images — shared field with NFT create; WebP strip + credit billing.
  </Card>
  <Card title="Real-Time Messaging" href="/docs/features/messaging">
    Product agent chats vs hidden generative tool chats (`type` vs `kind`).
  </Card>

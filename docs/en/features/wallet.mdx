...
<Cards>
  ...
  <Card title="Top-up — /wallet/topup" href="/docs/features/wallet">
    Add credits via native-token transfer (tx-hash verify) or live WayForPay card / Apple Pay / Google Pay (`wallet_topup` via PaymentConductor).
  </Card>
...
- **Top up credits** — native-token transfer (tx hash verify) **or** WayForPay card redirect via `initiateCreditTopupPayment` (amount 25–2000; webhook credits balance).
...
`initiateCreditTopupPayment` → `PaymentConductor.createCheckout({ purpose: 'wallet_topup' })` → `wayforpay.processor.ts` (`createWalletTopupWayForPay`) or Stripe → webhook `handlers/wallet-topup.ts` / `wallet-topup-stripe.ts` → `creditBalanceService.addFiatUsd` (1:1 fiat points). Amount gate: 25–2000. Card top-up credits the **fiat ledger only** — it never mints on-chain RING. Entry UI: `/wallet/topup` card tab (live) and `CreditAddFsModal`.
...
| `initiateTopUp` | Card → credit points (`wallet_topup`) |

...
<RelatedDocs>
  <RelatedArticle
    ...
    relation="Depends-on: live `credit:balance` and `wallet:list` updates while the wallet tab stays open."
  />
  <RelatedArticle
    slug="api/wallet"
    relation="Deep-dive: Server Actions and HTTP SSOT for list, credit, and transfer."
  />
  <RelatedArticle
    slug="customization/token-economics"
    relation="See-also: oracle rate, transfer tax, first-settler discounts."
  />
  <RelatedArticle
    slug="features/store"
    relation="Same-workflow: credit spend at checkout."
  />
  <RelatedArticle
    slug="features/authentication"
    relation="Prerequisite: Google/Apple/social sign-in triggers wallet provisioning."
  />
  <RelatedArticle
    slug="integrations/ethereum-wallets"
    relation="Deep-dive: Wagmi v3 pathNeedsWeb3 gate, getEvmRpcUrl / getEvmTokenAddress SSOT."
  />
  <RelatedArticle
    slug="features/public-pools"
    relation="Same-workflow: desk oracle nativePerMainCurrency converts public-pool card chip-ins to pledged native (not wallet_topup 1:1)."
  />
  <RelatedArticle
    slug="features/ring-oracle"
    relation="Depends-on: Ring Oracle facade for desk quotes, FX overlay, and credit accounting rates."
  />
</RelatedDocs>

...
<Callout type="info">
  Filter with **Founder** / **Developer** in the docs sidebar. This page reflects the verified v1.6.4 codebase after the 2026-07-03 SSOT refactor (PIN-wrapped encryption, atomic single-write provisioning, wallet_access_tokens token store, SSOT chain-identity consolidation).
</Callout>
...
| Oracle rate | `features/wallet/services/ring-token-oracle.ts` | `getRingPerUsdRate` — reads from `platform_settings` table |
...
Rate stored in `platform_settings.web3.oracle.ringPerUsd`. Managed via superadmin action `setRingPerUsdRate`.
...
| `ORACLE_QUOTE_SECRET` | For desk | `ring-token-oracle.ts` — desk quote HMAC signing |

Env vars documented here must be added manually to `.env.local` — they are not pre-configured in `env.local.template`. See also `lib/wallet/encrypt-wallet-secret.ts` for the v2 envelope format cryptographic spec.

...
The authorization check is conditional on `CRON_SECRET`, so production deployments must configure it. The response includes ProcessConductor `runId` plus refresh metadata; a disabled feed reports a successful skipped run.

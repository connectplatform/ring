...
- **Replicas** — default is single-pod in-process hub. Multi-replica publish needs `TUNNEL_HUB_MODE=k8s-postgres` **and** `TUNNEL_POSTGRES_FANOUT=enabled` (Postgres LISTEN/NOTIFY). Leave replicas at 1 until enabled + smoked. Redis/Connect hubs remain backlog.
...
- **Native WSS reconnect** — `NativeWsClient` self-heals unexpected closes (exponential backoff, fresh `/api/tunnel/token`, resubscribe retained channels). Emits `reconnect` while retrying; `disconnect` only on intentional close or exhausted attempts so TransportManager can fall back to SSE/poll.
...
# Multi-replica only — keep disabled until smoke:
...
<FutureFeatureWidget
  ...
  description="Shipped (default off): PostgresFanoutTunnelHub via TUNNEL_POSTGRES_FANOUT=enabled — live-only cross-pod delivery, stable message.id, DB_SSL-aware LISTEN. Leave replicas at 1 until enabled + smoked; Redis/Connect hubs remain backlog."
  implementationCost={48}
  labels={['tunnel', 'postgres', 'k8s', 'shipped']}
  voteCount={0}
/>

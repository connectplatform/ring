...
<Callout type="info">
  A `TunnelPublisher: Queued for user … (no live socket yet)` debug log right after login is expected, not an error. HTTP ingest (e.g. `POST /api/analytics/device`) persists first; tunnel connect follows a short auth-grace delay in `components/providers/tunnel-provider.tsx`. The message is queued and delivered on the next connect — see `lib/tunnel/SUBSCRIPTION-SSOT.md` "Tunnel timing + duplicate fetch consolidation".
</Callout>
...
| Per-user inbox | `credit:balance` | `creditBalanceService` | `CreditBalanceProvider` → `use-credit-balance.ts` (`userScoped: false`) |

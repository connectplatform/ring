...
| `011_platform_settings.sql` | `platform_settings` | Yes — runtime reads/writes via `db().*Doc` in `platform-settings-service.ts` and `ring-token-oracle.ts` (not raw SQL in feature modules) |

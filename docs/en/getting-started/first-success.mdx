...
**Expected:** JSON with `availableProviders`, `environment`, and `status: "ready"`.

**Notification test:**
1. Sign in and confirm SSE connects (browser devtools → Network → `/api/tunnel/sse`)
2. Create an opportunity or trigger a notification
3. Verify unread badge updates without waiting for the poll interval
...
- Set `RING_DEPLOY_TARGET` and `NEXT_PUBLIC_RING_DEPLOY_TARGET` (k8s/self-hosted for WSS)
- Check browser network tab for `/api/tunnel/ws` or `/api/tunnel/sse`
- Verify `/api/tunnel/test` returns `status: "ready"`
...
| **Real-time** | ☐ | Tunnel connected (WSS or SSE), notifications |
...
1. **[Explore Features](/en/docs/features)** - Discover all platform capabilities
2. **[Customize Branding](/en/docs/customization/branding)** - Make it your own
3. **[Add New Features](/en/docs/examples)** - Extend functionality
4. **[Deploy to Production](/en/docs/deployment)** - Share with the world
5. **[Join Community](https://discord.gg/ring-platform)** - Connect with other developers

**Need help customizing?** Check our [Customization section](/docs/customization) for operator and branding guides.
</Callout>

---

*Having issues? Check our [troubleshooting guide](/en/docs/getting-started/troubleshooting) or join our [Discord community](https://discord.gg/ring-platform) for support.*

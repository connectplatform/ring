...
import { NewsLikeButton } from '@/components/actions/like-button';
...
export default async function NewsArticlePage(props: LocalePageProps<NewsArticlePageParams>) {
  ...
  if (session?.user?.id && article.id) {
    try {
      const response = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/news/${article.id}/like`, {
        method: 'GET',
        headers: {
          ...
          <header className="mb-8">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4 pb-6 border-b">
              ...
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                ...
                <NewsLikeButton
                  targetId={article.id || ''}
                  initialLikeCount={article.likes || 0}

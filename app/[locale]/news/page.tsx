...
import { isValidLocale, defaultLocale, loadTranslations, generateHreflangAlternates } from '@/utils/i18n-server';
...
export async function generateMetadata(props: LocaleMetadataProps<NewsParams>): Promise<Metadata> {
  try {
    ...
    const title = translations.metadata?.news || translations.news?.title || 'News & Updates | Ring Platform';
    const description = translations.metaDescription?.news || translations.news?.description || 'Stay updated with the latest news, updates, and announcements from Ring Platform.';

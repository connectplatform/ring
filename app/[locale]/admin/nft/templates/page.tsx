...
export async function generateMetadata({
  ...
  return buildLocalizedMetadata({
    locale,
    path: 'admin',
    pathname: '/admin/nft/templates',
    robots: { index: false, follow: false },
  })
}

export default async function AdminNftTemplatesPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  return (
    <AdminWrapper locale={locale} pageContext="web3" labels={adminLabels}>
      <div className="p-4 sm:p-6 space-y-6">
        <div>
          <h1 className="text-2xl font-bold">NFT Gate Templates</h1>
          <p className="text-muted-foreground mt-1">
            Price and activate Metaplex Core template assets. GateEscrow stake unlocks features —
            not the APR staking pool.
          </p>
        </div>

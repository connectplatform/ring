/**
 * Cron Job: Expire abandoned peer game sessions + reclaim orphan pending invites.
 *
 * Vercel Cron path: /api/cron/peer-game-session-expiry
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const handler = getPipelineDefinition('peer-game-session-expiry')?.handler
    if (!handler) {

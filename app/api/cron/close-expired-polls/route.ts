/**
 * Cron Job: Close expired chat polls (TD-UX-03)
 *
 * Vercel Cron path: /api/cron/close-expired-polls
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const handler = getPipelineDefinition('close-expired-polls')?.handler
    if (!handler) {

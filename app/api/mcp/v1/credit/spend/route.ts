...
export const POST = withMcpGuard(async (request) => {
  ...
  if (body?.confirm !== true) return mcpError('credit spend requires confirm: true', 400)
  ...
  const result = await service.spendCredits(
    String(body.userId),
    {
      ...
      description: String(body.description || 'ring-mcp credit spend'),
      metadata: body.metadata as Record<string, unknown> | undefined,
    },
    (body.type as any) || 'purchase',

...
export async function getNewsLikeStatus(newsId: string): Promise<{ likeCount: number; isLiked: boolean }> {
  try {
    ...
    const newsResult = await db().readDoc<{ likes?: number }>('news', newsId)
    ...
    const likeCount = newsResult.data.likes ?? 0
    ...
    if (session?.user?.id) {
      ...
      isLiked = likeQueryResult.success && !!likeQueryResult.data && likeQueryResult.data.length > 0
    }

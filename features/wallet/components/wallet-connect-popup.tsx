    ...
    const fetchWalletData = async () => {
      if (isOpen && session) {
        ...
        if (sessionData?.user?.wallets && sessionData.user.wallets.length > 0) {
          ...
          const userWallet: Wallet = selectDefaultWallet(sessionData.user.wallets) || sessionData.user.wallets[0]
    ...
    >
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full mx-4">
        ...
        {!wallet ? (
          ...
          <div>
            <p><strong>{t('walletAddress')}:</strong> {formatAddress(wallet.address)}</p>
